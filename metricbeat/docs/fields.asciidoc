...
LDAP data and metrics collection
...
`ldap` contains metrics and data that were obtained from LDAP searches
...
ldapsearch will generate one event for every result item. To group them later, some fields are aggregated to identify related events



*`ldap.ldapsearch.search name`*::
+
--
